 ...
 *
 */
class BasketItemRepository extends \TYPO3\CMS\Extbase\Persistence\Repository {
	 ...
	 *
	 */
	public function findIsInBasket($sessionId, $ticketId){
		$query = $this->createQuery();
		$query->getQuerySettings()->setRespectStoragePage(FALSE);
		$query->matching(
			$query->logicalAnd(
				$query->equals('sessionId', $sessionId),
				$query->equals('ticket', $ticketId)
			)
		);
	 ...
	 *
	 */
	public function findBasketItem($sessionId, $ticketId){
		$query = $this->createQuery();
		$query->getQuerySettings()->setRespectStoragePage(FALSE);
		$query->matching(
			$query->logicalAnd(
